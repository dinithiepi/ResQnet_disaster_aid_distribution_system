...
function ItemRequestManagement() {
  ...
  const fetchItemRequests = async () => {
    try {
      ...
      const response = await fetch('http://localhost:4002/api/admin/item-requests', {
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch('http://localhost:4002/api/admin/item-requests/approve', {
        method: 'POST',
        headers: {
  ...
  const handleReject = async (requestId) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await fetch('http://localhost:4002/api/admin/item-requests/reject', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
