...
if (connectionString) {
  console.log('🔗 [Admin Service] Attempting to connect to Supabase using DATABASE_URL...');
  ...
  console.log('🏠 [Admin Service] Using local PostgreSQL connection (DATABASE_URL not set)...');
  pool = new Pool({
...
pool.on('connect', () => {
  console.log('✅ [Admin Service] Successfully connected to PostgreSQL database!');
...
pool.on('error', (err) => {
  console.error('❌ [Admin Service] Database connection error:', err.message);
