...
if (connectionString) {
  console.log('🔗 [Manager Service] Attempting to connect to Supabase using DATABASE_URL...');
  ...
  console.log('🏠 [Manager Service] Using local PostgreSQL connection (DATABASE_URL not set)...');
  pool = new Pool({
...
pool.on('connect', () => {
  console.log('✅ [Manager Service] Successfully connected to PostgreSQL database!');
...
pool.on('error', (err) => {
  console.error('❌ [Manager Service] Database connection error:', err.message);
